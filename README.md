...
- 当前V3联合训练将 simulation 的80个功率全部用于低保真训练，不再设置低保真验证或
  测试集；高保真仍固定为 Experiment `12/3` 训练/验证和 test_Data `3` 功率测试。
  simulation `60/10/10` 只保留为旧协议复现方式。
...
当前V3代码对应本地误差改进正式运行。训练连续完成8000轮、共120000次参数更新；
仅依据高保真验证集选中第400轮模型，验证集
顶部、热端、冷端逐功率平均RMSE分别为`3.180℃`、`1.035℃`、`0.779℃`，综合选择分数为
`2.302℃`。第8000轮综合分数为`2.846℃`，因此发布模型不是训练末态。

已查看测试集169、339、634 W仅用于锁定模型后的回归展示，不再用于调参。三个功率的
顶部、热端、冷端逐功率平均RMSE分别为`2.011℃`、`0.416℃`、`0.580℃`；其中634 W为
`3.207℃`、`0.500℃`、`0.518℃`。详细研究记录、图册、数据和模型权重保留在本地，
不属于本次GitHub代码发布内容。
...
旧固定协议的低保真检查点必须由60个 simulation train功率训练并带完整来源指纹。PRC先拟合
