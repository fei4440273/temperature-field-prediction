...
> 完整协议见 `DATA_USAGE_PLAN.md`。当前固定划分训练与测试结果见本文顶部；后续历史章节
> 中的 LOGO、旧全数据部署与旧数值仅作背景。

> 最后更新：2026-09-09 UTC
> 工程状态：`FIXED-SPLIT MULTIFIDELITY V2 / FINAL TEST COMPLETED`
> 当前阶段：MLP/DeepONet/硬表面引导已完成 5 种子独立测试；PRC 已完成 seed 0 先导对照。
> 当前运行：没有本项目训练任务正在运行；主机已有长期容器未被停止或修改。  
> 本文件用途：作为项目根目录的主进度看板，后续实施、训练和验证结果应同步更新到这里。

## 当前 V2 实施状态

- P0：543 文件来源清单、SHA-256、processed 指纹、解析物理快照和协议审计已生成。
- P1：连续坐标直查、`macro_v1`、时间窗/峰值指标、rank-0 完整验证和物理语义测试已实现。
- P2：普通多保真 joint 默认由 LF 输出拟合仿真；两材料消费量、梯度和冻结状态进入日志。
- P3：PRC LF 全域响应及 V/A/AT 校正、训练入口和保存/重载支持已实现并完成 seed 0 先导训练。
- P4：测试读取要求冻结 release manifest；训练和旧评估入口不能直接查看 test_Data。
- 测试集已在模型选择完成后按用户授权读取，后续不得根据测试结果调参或重新选模。

## 当前固定测试结论

- 独立测试功率：169、339、634 W；训练、验证、测试按完整功率及 Top/Hot/Cold 联动隔离。
- DeepONet-PINN 五种子总体 RMSE/MAE：`1.9600 ± 0.2743℃ / 1.6947 ± 0.2790℃`。
- MLP-PINN 五种子总体 RMSE/MAE：`2.4361 ± 0.5547℃ / 2.1881 ± 0.5774℃`。
- 硬表面引导五种子总体 RMSE/MAE：`3.1736 ± 0.0705℃ / 3.0127 ± 0.0683℃`。
- PRC-AT seed 0 RMSE/MAE：`2.0404℃ / 1.6805℃`；它没有超过 DeepONet-PINN seed 0。
- PRC 是“极点-留数校正”；V/A/AT 分别表示温度残差、幅值、幅值与时间尺度校正。
- 完整三功率和分模态结果：`reports/final_test_20260909_user_authorized/README.md`。

对外温度、图表和误差指标统一使用 ℃；物理方程内部保留 K，以保证辐射和物性计算正确。
...
`MAE=7.734 +/- 1.152 ℃`，未通过门槛。新增功率结构化和表面残差插值消融后，最终锁定
**DeepONet 低保真体场 + 硬表面绝对温度引导 + 材料感知 PINN 内部修正**。

最终候选按每次留出一个完整实验功率的 **10 折 LOGO x 5 seeds** 运行 50 个模型。折外
SiC 顶面 `RMSE=4.635 ℃`、`MAE=3.626 ℃`、最高温度相对误差 `2.412%`；Hot/Cold 环
绝对/温升 RMSE 为 `3.321 +/- 0.256 ℃` 和 `2.395 +/- 0.080 ℃`。平均表面 MAE 和峰值
门槛通过，但 115.2 W 与 558.5 W 的逐功率表面 MAE 分别为 `5.144 ℃` 和 `7.374 ℃`，
故“每个功率均小于 5 ℃”仍未通过。表面引导确定后不依赖神经网络随机种子，所以表面指标
...
| Hot/Cold 验证 | 已完成 | 最终 LOGO 绝对/温升 RMSE 为 3.321/2.395 ℃ |
| 完整功率交叉验证 | 10 折 LOGO x 5 seeds 完成 | 50 个模型；表面 MAE 3.626 ℃，平均门槛通过 |
...
- 所有功率使用同一网格。
- 温度范围：`22.0-533.579 ℃`。
- 所有 80 个功率、共 92,720 个 `t=0` 节点均为 `22 ℃`，单文件标准差为 0。
...
当前唯一有效划分见本文顶部、`DATA_USAGE_PLAN.md` 与 `configs/splits.yaml`。本节只保留旧
实验记录，不能据此恢复 5 折、LOGO 或旧 external sensor 分组。
...
- 稳定时间：从某帧开始，后续所有最高温度相邻帧变化均不超过 `0.01 ℃`。
- 0 W：返回所有仿真共同观测到的 `22℃` 均匀初始场；环境与水冷温度均已确认为 22 ℃。
...
- Seed 0 验证集窗口 8/16/32 的最佳 RMSE：`0.906/0.994/0.725 ℃`。
- 按验证集锁定窗口 32，测试集不参与窗口选择。
- 窗口 32 的最高温度曲线 MAE：`2.116 +/- 0.108 ℃`。
- DeepONet 对应值：`1.011 +/- 0.378 ℃`。
- Global POD 对应值：`0.355 +/- 0.060 ℃`。
...
| 确定性 MF 表面残差 | 径向可靠度加权，5 seeds | 5.53 +/- 0.52 | 4.51 +/- 0.44 | 达到平均表面 MAE 5 ℃ 门槛 |
| LF FEM 线性插值 | 原始像素分级聚合 | 29.92 +/- 10.60 | 26.59 +/- 9.60 | 未做实验校正 |
| 确定性 MF 表面残差 | 原始像素分级聚合，5 seeds | 5.75 +/- 0.41 | 5.33 +/- 0.51 | 未达到像素口径 MAE 5 ℃ |
...
- Seed 4 在 593.5 W 的 MAE 为 `6.13 ℃`，说明随机种子稳定性仍需改进。
...
|---|---:|---:|---:|---|
| SiC 顶面 RMSE | 4.635 ℃ | 0 | 0 | 参考指标 |
| SiC 顶面 MAE | 3.626 ℃ | 0 | 0 | `<=5 ℃` 通过 |
| 最高温度绝对误差 | 3.765 ℃ | 0 | 0 | 参考指标 |
| 最高温度相对误差 | 2.412% | 0 | 0 | `<=5%` 通过 |
| Hot/Cold 绝对 RMSE | 3.321 ℃ | 0.256 ℃ | 0.318 ℃ | 内部稀疏观测 |
| Hot/Cold 温升 RMSE | 2.395 ℃ | 0.080 ℃ | 0.099 ℃ | 内部稀疏观测 |

表面指标的 seed 标准差为 0 是硬引导确定性的预期结果，不代表物理试验没有不确定性。逐功率
仍有两个 MAE 超过 5 ℃：115.2 W 为 `5.144 ℃`，558.5 W 为 `7.374 ℃`；115.2 W 的峰值
相对误差也为 `7.583%`。因此只判定“跨功率平均门槛通过”，不判定“每个功率均通过”。
完整证据位于 `reports/mf_pinn_hard_surface_temperature_logo_5seed_summary.json`。

仅预测 SiC 表面时，`regression_residual_per_watt` 的 LOGO MAE 略低，为 `3.591 ℃`，但它
...
旧版 `sensor_absolute=0.2` 仅按 IR 选检查点，虽然表面 MAE 为 `4.914 ℃`，主测试铜环绝对
RMSE 高达 `20.469 ℃`。该结果保留为开发基线，不能作为当前候选。
...
|---|---:|---:|---:|---|
| 验证综合分数 | 3.251 ℃ | 0.270 ℃ | 0.335 ℃ | 仅用于模型选择 |
| 验证 IR RMSE | 4.592 ℃ | 0.274 ℃ | 0.340 ℃ | 仅用于模型选择 |
| 测试 IR RMSE | 6.674 ℃ | 0.784 ℃ | 0.973 ℃ | 待改善 |
| 测试 IR MAE | 5.111 ℃ | 0.583 ℃ | 0.724 ℃ | `<=5 ℃` 未通过 |
| 测试峰值相对误差 | 1.266% | 0.196% | 0.243% | `<=5%` 通过 |
| 主测试铜环绝对 RMSE | 4.053 ℃ | 0.173 ℃ | 0.214 ℃ | 明显改善 |
| 主测试铜环温升 RMSE | 2.749 ℃ | 0.257 ℃ | 0.319 ℃ | 原型水平 |

按测试功率聚合：

| 功率 | IR RMSE | IR MAE | 峰值相对误差 | 判定 |
|---:|---:|---:|---:|---|
| 309 W | 4.779 +/- 0.499 ℃ | 3.384 +/- 0.637 ℃ | 1.025 +/- 0.413% | 通过 |
| 593.5 W | 7.950 +/- 1.118 ℃ | 6.615 +/- 0.758 ℃ | 1.507 +/- 0.061% | MAE 未通过 |

7 个完全外部 Hot/Cold 功率（50/250/350/500/600/700/800 W）的 5-seed 结果为：绝对
温度 RMSE `7.015 +/- 0.758 ℃`，温升曲线 RMSE `4.584 +/- 1.023 ℃`。完整结果见
...
| 指标 | 5-seed 均值 | seed 标准差 | 95% CI 半宽 | 判定 |
|---|---:|---:|---:|---|
| 表面 RMSE | 9.140 ℃ | 1.302 ℃ | 1.617 ℃ | 未通过 |
| 表面 MAE | 7.734 ℃ | 1.152 ℃ | 1.430 ℃ | `<=5 ℃` 未通过 |
| 峰值相对误差 | 4.088% | 0.531% | 0.660% | 平均通过，115.2 W 失败 |
| 铜环绝对 RMSE | 3.430 ℃ | 0.870 ℃ | 1.080 ℃ | 原型水平 |
| 铜环温升 RMSE | 2.408 ℃ | 0.653 ℃ | 0.810 ℃ | 原型水平 |

跨 seed 的表面 MAE 较大功率为 115.2 W `8.369 ℃`、558.5 W `8.587 ℃`、593.5 W
`13.627 ℃` 和 630.5 W `19.101 ℃`；309 W 也为 `5.594 ℃`。该配置的跨功率泛化明确
...
- 固定 low/mid/high 辐射率场景各训练 400 轮后，最佳验证 RMSE 为
  `4.371848/4.369855/4.366256 ℃`，最大差仅 `0.005592 ℃`，低于 `0.1 ℃` 判定阈值。
...
- 全部仿真中的最大材料侧温差为 `95.90 ℃`，位置为 800 W、158 s、
  `(r,z)=(0.025,0) m`。
- 这否定了“可以直接默认界面温度连续”的假设，但没有界面热流，因此不能仅由温差反推出
  接触热阻。
- 无材料标签 MLP 在同坐标只能输出相同温度，冻结测试集的界面温差预测恒为 0，温差
  MAE 为 `31.67 ℃`。
- 加入 `material_id` 后，界面温差 MAE：MLP `1.63 +/- 0.16 ℃`，LSTM
  `1.571 +/- 0.070 ℃`，DeepONet `0.930 +/- 0.075 ℃`。
...
- `t=0` 全场严格为 `22.0 ℃`；0-200 s 全场范围 `22.0-44.240 ℃`。
- 铜外圆柱面共 19 个节点，全时刻固定温度最大误差 `0 ℃`；全场数值有限且不低于 22 ℃。
- 200 s 的最高温度为 `44.240 ℃`，最后 1 s 仍变化 `0.0286 ℃`，故 200 s 内未满足
  `0.01 ℃` 稳定判据；接口返回 `None`，报告记为 `>200 s`。
...
- `t=0` 全部 1,159 个节点严格为 `22.0 ℃`。
- 0-200 s 全场范围：`22.0-42.586 ℃`；铜外圆柱面全时刻严格为 `22.0 ℃`。
- 按后续所有相邻 Tmax 帧变化均不超过 `0.01 ℃` 的规则，稳定时刻为 `140 s`。
- 输出包括 101 帧轴对称 NPZ、Hot/Cold CSV、5 个时刻的 72 角向 VTK 和 PNG。
- 36 W 低于 IR 最小功率，实验残差按功率距离二次收缩到低保真先验，并保留外推警告。
- 部署投影还强制全场温度不低于 22 ℃；该投影不进入训练图，避免 softplus 饱和造成塌缩。
- 状态：历史原型，已由上面的全数据最佳部署检查点取代。

未约束检查点曾在 36 W 给出 `t=0` 约 `16.36-41.24 ℃` 且 Tmax 明显回落，已保留在
...
- 限制：这是基于已确认 `22 ℃` 初始/环境/水冷条件的解析锚点，不是激光开启后的实验验证场。

### 不可用于部署的 MLP 示例

- `predictions/36W_mlp_seed0/`：最低约 `9.17 ℃`，200 s 内未稳定。
- `predictions/36W_mlp_material_seed0/`：最低约 `7.58 ℃`，200 s 内未稳定。
- 两者均违反已观测的 22 ℃ 初始场，只保留为失败基线证据。
...
- 最终 10 折 LOGO x 5 seeds 的 50 个模型均完成；表面 MAE 3.626 ℃、峰值相对误差
  2.412%，平均门槛通过。
...
初始温度、环境温度和水冷固定温度均为 `22℃`。水冷只施加到铜外圆柱面
`r=0.05834 m`，不施加到底面。
...
场景的验证 RMSE 最大差仅 `0.005592 ℃`。后续名义训练采用 `0.5/0.5` 中性值并报告两端
敏感性，不把它们写成材料实测值。
...
- `r_mm` 与 `sqrt(x_mm^2+y_mm^2)` 的最大误差仅 `1.846e-6 mm`，故文件坐标中心按
  `(0,0) mm` 使用。
- 正式物理初始温度为 `22 ℃`；IR 首个已保存温度场位于激光开启后 `5 s`，不冒充 `t=0`。
...
| 完整多保真训练 | `SURFACE_MEAN_GATES_PASSED` | 最终 LOGO 表面 MAE 3.626 ℃；内部场无实验真值 |
| Hot/Cold 元数据加载 | `PASS` | 米、摄氏度、逐秒时间和激光开启同步已确认 |
| Hot/Cold 外部评估 | `COMPLETE_PROTOTYPE` | 7 功率绝对 RMSE 7.015 ℃、温升 RMSE 4.584 ℃ |
| 10 折 LOGO 功率验证 | `5SEED_COMPLETE_MEAN_ACCEPTED` | 50 模型，表面 MAE 3.626 ℃；逐功率门槛未全部通过 |
...
|---|---|
| 项目说明 | `README.md` |
| 当前 V2 方案 | `temperature_field_prediction_multifidelity_plan_v2.md` |
...
| 历史部署检查点（非当前协议） | `reports/deployment/mf_pinn_hard_surface_temperature_all_data_seed0_constrained.pt` |
