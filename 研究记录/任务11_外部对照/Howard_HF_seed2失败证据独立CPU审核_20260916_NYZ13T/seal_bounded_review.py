...
record_path = here / "独立CPU只读再核.command.json"
...
stdout_path = here / "独立CPU只读再核.stdout.txt"
stderr_path = here / "独立CPU只读再核.stderr.txt"
...
if check:
    ...
    names = ("independent_failure_audit.py", "run_bounded_readonly.py", "seal_bounded_review.py",
             ...
             "独立CPU只读再核.command.json", "独立CPU只读再核.stdout.txt", "独立CPU只读再核.stderr.txt",
             "主validator首轮原件.py", "封存器首轮原件.py", "自身封存首轮失败补录.json", "dirfd_fixture.txt",
             "test_guard_dir_fd_readonly.py", "run_guard_dir_fd_probe.py", "自身guard_dir_fd_RED.command.json", "自身guard_dir_fd_GREEN.command.json")
    environment = record["实际环境"]
    first = audit.read_json(here / "独立CPU只读实跑.command.json")
    assert first["实际退出码"] == 0 and "_exitfunc" in first["完整stderr"] and "dir_fd" in first["完整stderr"]
    red = audit.read_json(here / "自身guard_dir_fd_RED.command.json")
    green = audit.read_json(here / "自身guard_dir_fd_GREEN.command.json")
    assert red["实际退出码"] == 1 and green["实际退出码"] == 0 and green["完整stderr"] == ""
    first_tmp = here / "临时文件/tmp2mij6hq5/_remote_module_non_scriptable.py"
    assert first_tmp.is_file()
    value = {"性质": "独立只读有界seed2失败证据审核封存，非正式训练完成、模型复现、能源/OBS或全目标验收",
        ...
        "首轮实际exit0和退出期nonempty完整stderr": {"原退出码": first["实际退出码"], "原完整stderr": first["完整stderr"],
            "stdout守卫计数时点": "主print之前，不冒退出后全时，退出时自身dir_fd误解析另有真实stderr",
            "首次封存器actualexit": 1, "首轮异常临时文件仍保留": audit.identity(first_tmp)},
        "仅自身新guard相对dir_fd修复真实RED_GREEN": {"RED": red, "GREEN": green},
