...
def io_frames(tmp_path_factory):
    ...
    for power, count, ring_count in ((115.2, 19, 100), (403., 24, 126), (630.5, 29, 150)):
        ...
        for index, time in enumerate(times):
            ...
            raw64 = native["reliability_weight_raw"].to_numpy().astype(np.float64)
            expected = raw64 / raw64.sum()
            np.testing.assert_allclose(native["frame_weight"].to_numpy(), expected, rtol=1e-6, atol=1e-9)
