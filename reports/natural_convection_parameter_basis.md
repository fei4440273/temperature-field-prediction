...
铜外圆柱面 `r=0.05834 m` 使用用户确认的 `295.15 K` 固定温度水冷边界，不再叠加自然对流。
...
为满足当前“常物性”设定，计算固定使用约 300 K 的空气参数：`k=0.0263 W/(m*K)`、
`nu=15.89e-6 m^2/s`、`alpha=22.5e-6 m^2/s`、`beta=1/300 K^-1`。空气输运物性的权威
数据源类型为 NIST REFPROP；这里的常数化是工程近似，不是 REFPROP 全温区逐点求值。

使用全部 80 个 Simulation 功率的水平表面面积加权平均温度，并排除小于 `1 K` 的温差后：

| 表面 | 样本数 | 温差 P05/中位/P95/最大 (K) | h P05/中位/P95/最大 (W/(m^2*K)) |
|---|---:|---|---|
